...
function BlogList({ blog }) {
    ...
    useEffect(() => {
        const fetchLikesAndDislikes = async () => {
            ...
            try {
                const likeResponse = await axios.get(`http://localhost:5000/api/v1/isLike`, {
                ...
                const dislikeResponse = await axios.get(`http://localhost:5000/api/v1/isDisLike`, {
                    withCredentials: true,
                    params: { postId: blog._id },
                });
    ...
    const handleLike = async () => {
        try {
            if (isLike) {
                await axios.post(`http://localhost:5000/api/v1/removeLike`, {
                    postId: blog._id,
                    likeId,
                }, { withCredentials: true });
                setIsLike(false);
                ...
                const response = await axios.post(`http://localhost:5000/api/v1/giveLike`, {
                    post: blog._id,
                }, { withCredentials: true });
                setIsLike(true);
    ...
    const handleDislike = async () => {
        try {
            if (isDislike) {
                await axios.post(`http://localhost:5000/api/v1/removeDisLike`, {
                    postId: blog._id,
                }, { withCredentials: true });
                setIsDislike(false);
                ...
                const response = await axios.post(`http://localhost:5000/api/v1/addDisLike`, {
                    postId: blog._id,
                }, { withCredentials: true });
                setIsDislike(true);
